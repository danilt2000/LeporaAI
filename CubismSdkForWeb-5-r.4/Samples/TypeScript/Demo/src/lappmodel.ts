...
export class LAppModel extends CubismUserModel {
  ...
  private _idMouthForm: CubismIdHandle;
...
//     // Mouth open
//     const speedHz  = 0.8;    // можно делать меньше → медленнее
// // амплитуда (максимум = 1 − minOpen)
// const minOpen  = 0.0;
// const amplitude = 1.0 - minOpen;

// // синусоидальный расчёт
// const t    = this._userTimeSeconds;
// const sinv = Math.sin(2 * Math.PI * speedHz * t);
// const mouthOpen = minOpen + amplitude * ((sinv + 1) / 2);

// // применяем
// this._model.setParameterValueById(this._idMouthOpenY, mouthOpen);
    
// Optional: add a little shape (e.g. slight smile)
// const formValue = 0.2;  // –1…1
// this._model.setParameterValueById(this._idMouthForm, formValue);
  
const speedHz     = 0.8;
const minOpen     = 0.0;
const ampY        = 1.1 - minOpen;
const widthFactor = 1.5;  // сила «растяжения» по бокам

// расчёт
const t    = this._userTimeSeconds;
const norm = (Math.sin(2 * Math.PI * speedHz * t) + 1) / 2;

// вертикальное раскрытие
const mouthOpenY = minOpen + ampY * norm;
// «форма» рта — ширина
const mouthForm = norm * widthFactor;
// ограничим, если нужно, чтобы форма не вылазила за пределы [0;1]
const clampedForm = Math.min(Math.max(mouthForm, 0), 1);

// применяем оба
this._model.setParameterValueById(this._idMouthOpenY, mouthOpenY);

const formValue = 0.5;  // –1…1
this._model.setParameterValueById(this._idMouthForm, formValue);
  ...
  public constructor() {
    ...
    this._idMouthOpenY = CubismFramework.getIdManager().getId("ParamMouthOpenY");
    this._idMouthForm = CubismFramework.getIdManager().getId("ParamMouthForm");
